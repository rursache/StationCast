...
| `STATIONCAST_LYRICS` | `true` | Look up lyrics for the playing track and cache them. LRCLIB first, which is the only free source with time-synced lyrics, falling back to lyrics.ovh for plain text when LRCLIB has no match. The lyrics button only appears when a match is found. Set to `false` to make no third-party lyrics requests at all |
