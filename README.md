...
| `STATIONCAST_LYRICS` | `true` | Look up lyrics for the playing track and cache them. Sources are tried in order of quality: LRCLIB, the only free one with time-synced lyrics, then lyrics.ovh and versuri.ro for plain text. versuri.ro is last because it is parsed from pages rather than an API, and it is there because it carries Romanian music the others miss. The lyrics button only appears when a match is found. Set to `false` to make no third-party lyrics requests at all |
