...
| `STATIONCAST_LYRICS` | `true` | Look up lyrics for the playing track on LRCLIB and cache them. The lyrics button only appears when a match is found. Set to `false` to make no third-party lyrics requests at all |
